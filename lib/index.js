...
export function apply(ctx) {
  ...
  ctx.effect(() => {
    ...
    disposers.push(ctx.on('agent/session-start', (payload) => {
      ...
      setSession(sessionOf(payload && payload.agent), 'starting', '启动中')
    ...
    disposers.push(ctx.on('tools/result', (exec) => {
      ...
      agentState.lastEvent = 'tools/result:' + (tname || '?')
      agentState.at = Date.now()
    }))
        ...
        try {
          ...
          if (pathname === '/dsh-pets/api/mode' && method === 'POST') {
            ...
            if (nextMode === 'desktop') {
              ...
              try {
                spawn(exe, ['.'], { cwd: DESKTOP_DIR, detached: true, stdio: 'ignore', windowsHide: true })
